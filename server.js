/**
 * ═══════════════════════════════════════════════════════════════════════════
 * EXPANDSPAIN ALPHA™ - MAIN SERVER v3.2.0 (DEBUG COMPLETO)
 * ═══════════════════════════════════════════════════════════════════════════
 * 
 * CHANGELOG v3.2.0:
 * ✅ Logs de debug completos em GENERATE_REPORT
 * ✅ Mostra EXATAMENTE quando IA é chamada ou não
 * ✅ Detecta sessões completadas (cache do banco)
 * ✅ Corrigido erro "Column count doesn't match value count" (v3.1.1)
 * 
...
const diagnoseLimiter = rateLimit({ 
    windowMs: 60 * 60 * 1000, 
    max: 30, 
    message: { success: false, error: 'Too many diagnosis requests.', code: 'DIAGNOSIS_LIMIT_EXCEEDED' } 
});

const verificationLimiter = rateLimit({ 
    windowMs: 15 * 60 * 1000, 
    max: 10, 
    message: { success: false, error: 'Too many verification attempts.', code: 'VERIFICATION_LIMIT_EXCEEDED' } 
});

const resendLimiter = rateLimit({ 
    windowMs: 60 * 60 * 1000, 
    max: 5, 
    message: { success: false, error: 'Too many code resend requests.', code: 'RESEND_LIMIT_EXCEEDED' } 
});
...
app.use((req, res, next) => { 
    req.setTimeout(30000, () => res.status(408).json({ 
        success: false, 
    ...
    try {
        ...
        if (action === 'VERIFY_CODE') {
            ...
            const [sessions] = await pool.execute(
                `SELECT session_id, access_code, created_at, email_verified 
        ...
        if (action === 'RESEND_CODE') {
            ...
            const [sessions] = await pool.execute(
                `SELECT session_id, email, first_name, language, email_verified 
                FROM alpha_diagnoses 
                WHERE session_id = ? LIMIT 1`, 
                [sessionId]
            );
            ...
            await pool.execute(
                `UPDATE alpha_diagnoses 
                SET access_code = ?, created_at = NOW(), updated_at = NOW() 
                WHERE session_id = ?`, 
        ...
        if (action === 'RESPONSE') {
            ...
            const [sessions] = await pool.execute(
                `SELECT session_id, answers_json, completed_at, language, 
                current_question_index, email_verified 
                FROM alpha_diagnoses 
                WHERE session_id = ? LIMIT 1`, 
                [sessionId]
            );
            ...
            await pool.execute(
                `UPDATE alpha_diagnoses 
                SET answers_json = ?, current_question_index = ?, updated_at = NOW() 
                WHERE session_id = ?`, 
                [JSON.stringify(answers), nextIndex, sessionId]
            );
            ...
            return res.json({ 
                ...
                progress: { current: nextIndex, total: 20 } 
            });
        }

        // ====================================================================
        // ACTION: GENERATE_REPORT (COM LOGS COMPLETOS DE DEBUG)
        // ====================================================================
        if (action === 'GENERATE_REPORT') {
            console.log('═'.repeat(70));
            console.log('📊 [SERVER] GENERATE_REPORT INICIADO');
            console.log('═'.repeat(70));
            ...
            console.log(`🔍 [SERVER] Buscando sessão no banco...`);
            console.log(`   SessionId: ${sessionId}`);
            console.log(`   Language solicitado: ${language || 'não especificado'}`);
            
            const [sessions] = await pool.execute(
                `SELECT * FROM alpha_diagnoses WHERE session_id = ? LIMIT 1`, 
                [sessionId]
            );
            
            if (sessions.length === 0) {
                console.error(`❌ [SERVER] Sessão não encontrada: ${sessionId}`);
                return res.status(404).json({ 
                    success: false, 
                    error: 'Session not found' 
                });
            ...
            console.log(`✅ [SERVER] Sessão encontrada`);
            console.log(`   Email: ${session.email}`);
            console.log(`   Email verificado: ${session.email_verified}`);
            console.log(`   Completed_at: ${session.completed_at || 'NULL (nunca completado)'}`);
            console.log(`   Score atual: ${session.score || 'NULL'}`);
            console.log(`   AI Analysis length: ${session.ai_analysis?.length || 0} caracteres`);
            
            if (!session.email_verified) {
                console.error(`❌ [SERVER] Email não verificado`);
                return res.status(403).json({ 
                    success: false, 
                    error: 'Email not verified', 
                    code: 'EMAIL_NOT_VERIFIED' 
            ...
            // ================================================================
            // VERIFICAÇÃO CRÍTICA: RELATÓRIO JÁ EXISTE?
            // ================================================================
            if (session.completed_at) {
                console.log('⚠️  [SERVER] SESSÃO JÁ COMPLETADA ANTERIORMENTE!');
                console.log(`   Data de conclusão: ${session.completed_at}`);
                console.log(`   Score salvo: ${session.score}`);
                console.log(`   Status salvo: ${session.status}`);
                console.log(`   ❌ IA NÃO SERÁ CHAMADA (retornando dados do banco)`);
                console.log('═'.repeat(70));
                ...
                return res.json({ 
                    ...
                    message: 'Report already generated (cached from database)' 
                });
            }
            ...
            // ================================================================
            console.log(`🔄 [SERVER] Sessão NUNCA foi completada, gerando NOVO relatório...`);
            
            const answers = JSON.parse(session.answers_json || '{}');
            const reportLanguage = language || session.language || 'pt';
            
            console.log(`   Respostas encontradas: ${Object.keys(answers).length} perguntas`);
            console.log(`   Idioma do relatório: ${reportLanguage}`);
            
            console.log(`\n📊 [SERVER] Calculando score...`);
            const scoreData = scoring.calculateScore(answers, reportLanguage);
            
            console.log(`✅ [SERVER] Score calculado`);
            ...
            console.log(`\n🤖 [SERVER] Chamando generateAIAnalysis()...`);
            console.log(`   (Aguardando resposta da IA...)`);
            
            const aiAnalysis = await generateAIAnalysis(scoreData, answers, reportLanguage);
            
            console.log(`\n✅ [SERVER] IA retornou análise`);
            console.log(`   Tamanho: ${aiAnalysis.length} caracteres`);
            console.log(`   Primeiros 100 chars: ${aiAnalysis.substring(0, 100)}...`);
            ...
            console.log(`\n💾 [SERVER] Salvando no banco...`);
            
            await pool.execute(
                `UPDATE alpha_diagnoses 
                SET score = ?, status = ?, status_color = ?, report_json = ?, 
                ai_analysis = ?, completed_at = NOW(), updated_at = NOW() 
                WHERE session_id = ?`, 
                [
                    scoreData.score, 
                    scoreData.status, 
            ...
            console.log(`✅ [SERVER] Dados salvos no banco`);
            
            let emailSent = false, whatsappSent = false;
            
            console.log(`\n📧 [SERVER] Enviando email de diagnóstico...`);
            try {
                await sendDiagnosisEmail(
                    { email: session.email, firstName: session.first_name }, 
                    scoreData, 
                    aiAnalysis, 
                    session.access_code, 
                    reportLanguage
                ...
                emailSent = true;
                console.log(`✅ [SERVER] Email enviado com sucesso`);
            } catch (e) { 
                console.error('❌ Erro ao enviar email de resultado:', e.message); 
            }
            
            if (session.whatsapp) {
                console.log(`\n📱 [SERVER] Enviando WhatsApp...`);
                try {
                    if (await sendResultNotification(
                        { firstName: session.first_name, whatsapp: session.whatsapp }, 
                        scoreData, 
                        session.access_code, 
                        reportLanguage
                    )) {
                        whatsappSent = true;
                        console.log(`✅ [SERVER] WhatsApp enviado`);
                    }
                } catch (e) { 
                    console.error('❌ Erro ao enviar WhatsApp:', e.message); 
                }
            } else {
                console.log(`⚠️  [SERVER] WhatsApp não configurado`);
            }
            
            console.log('\n' + '═'.repeat(70));
            console.log('✅ [SERVER] GENERATE_REPORT CONCLUÍDO COM SUCESSO');
            console.log('═'.repeat(70));
        ...
        console.error('❌ ERRO FATAL no /api/diagnose');
        console.error('═'.repeat(70));
...
async function startServer() {
    try {
        ...
        app.listen(PORT, () => {
            console.log('═'.repeat(70));
            console.log(`🚀 EXPANDSPAIN ALPHA™ v3.2.0 - BACKEND COM DEBUG`);
